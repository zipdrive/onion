...
#define SPRITE_KEY BUFFER_KEY
	...
	struct Sprite
	{
		...
		// This should be equal to the start index of the sprite's vertex attributes in the buffer, divided by the total number of primitives corresponding to a single vertex in the buffer array.
...
#define RGBA_INT_TO_FLOAT	0.00392157f
	...
	protected:
		...
		/// <summary>Loads vertex attribute data from a meta file.</summary>
		/// <param name="file">The meta file containing vertex attribute data.</param>
		/// <param name="image">The loaded image.</param>
		/// <returns>The vertex buffer data generated by the meta file.</returns>
		virtual opengl::_VertexBufferData* __load(LoadFile& file, opengl::_Image* image) = 0;
	...
	{
	protected:
		typedef Shader<FLOAT_MAT4, _Args...> _SpriteShader;
	...
	public:
		...
		virtual void load(const char* path)
		{
			...
			// Load the image
			opengl::_Image* image = load_image(path);

			// Construct the path to the meta file.
			String fpath("res/img/");
			fpath.append(path);
			std::regex fext_finder("(.*)\\.[^\\.]+"); // Regex to find the path excluding file extension
			fpath = regex_replace(fpath, fext_finder, "$1.meta");

			// Load the file
			LoadFile file(fpath);
			opengl::_VertexBufferData* data = __load(file, image);
			...
			// Free the data buffer now that it's been stored in memory
			delete data;
		...
		/// <param name="args">Values to pass to the shader.</param>
		void display(const Sprite* sprite, const _Args&... args) const
		{
			// Activate the shader
			m_Shader->activate(Transform::model.get(), args...);
	...
	class SimplePixelSpriteSheet : public PixelSpriteSheet<FLOAT_MAT4, Int>
	{
	protected:
	...
	class ShadedTexturePixelSpriteSheet : public PixelSpriteSheet<FLOAT_MAT4X2, FLOAT_MAT4, FLOAT_MAT4, FLOAT_MAT4, Int>
	{
	protected:
