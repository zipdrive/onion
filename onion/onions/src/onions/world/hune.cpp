...
#include <algorithm>
#include <set>
#include "../../../include/onions/world/hune.h"
...
namespace onion
{
	namespace world
	{
		...
		template <>
		struct HuneAnimationValues<HUNE_STANDING>
		{
		...
		template <>
		struct HuneAnimationValues<HUNE_WALKING>
		{
		...
		template <>
		struct HuneAnimationValues<HUNE_ALL>
		{
			static constexpr int index = 0;
			static constexpr int size = HuneAnimationValues<HUNE_STANDING>::size + HuneAnimationValues<HUNE_WALKING>::size;
		};
		...
		public:
			HuneStaticShading(HuneBodyPart body_part, string prefix)
			{
				...
				m_Sprites[FACING_FRONT].sprite = HuneSprite::sprite_sheet->get_sprite(prefix + " front");
				m_Sprites[FACING_FRONT].flip_horizontally = body_part == LEFT_EAR;

				m_Sprites[FACING_BACK].sprite = HuneSprite::sprite_sheet->get_sprite(prefix + " back");
				m_Sprites[FACING_BACK].flip_horizontally = body_part == RIGHT_EAR;

				m_Sprites[FACING_RIGHT].sprite = body_part == LEFT_EAR ? nullptr : HuneSprite::sprite_sheet->get_sprite(prefix + " right");
				m_Sprites[FACING_RIGHT].flip_horizontally = false;

				m_Sprites[FACING_LEFT].sprite = body_part == RIGHT_EAR ? nullptr : HuneSprite::sprite_sheet->get_sprite(prefix + " right");
			...
			{
				if (frame >= HuneAnimationValues<HUNE_ALL>::size) return nullptr;
		...
		protected:
			...
			HuneSprite m_Sprites[HUNE_NUM_DIRECTIONS * HuneAnimationValues<HUNE_ALL>::size];
			...
			{
				set_frame(facing, HuneAnimationValues<HUNE_STANDING>::index, prefix + "walk0", false);

				int mid_index = HuneAnimationValues<HUNE_WALKING>::size / 2;
				for (int refl = mid_index; refl >= 0; refl -= mid_index)
				{
					for (int k = mid_index - 1; k >= 0; --k)
						set_frame(facing, HuneAnimationValues<HUNE_WALKING>::index + refl + k, prefix + "walk" + to_string(k > HuneAnimationValues<HUNE_WALKING>::size / 4 ? mid_index - k : k), refl > 0);
				}
			}
			...
			{
				set_frame(facing, HuneAnimationValues<HUNE_STANDING>::index, prefix + "walk0", flip_horizontally);

				if (half_displacement)
				{
					int quarter_size = HuneAnimationValues<HUNE_WALKING>::size / 4;
					int half_size = HuneAnimationValues<HUNE_WALKING>::size / 2;

					for (int k = half_size - 1; k > 0; --k)
						set_frame(
							facing, 
							HuneAnimationValues<HUNE_WALKING>::index + k, 
							prefix + "walk" + to_string(k > quarter_size ? (3 * quarter_size) - k : k + quarter_size), 
							flip_horizontally
						);
					for (int k = HuneAnimationValues<HUNE_WALKING>::size; k >= half_size; --k)
						set_frame(
							facing, 
							HuneAnimationValues<HUNE_WALKING>::index + (k % HuneAnimationValues<HUNE_WALKING>::size), 
							prefix + "walk" + to_string(k > 3 * quarter_size ? HuneAnimationValues<HUNE_WALKING>::size - k : k - half_size),
							flip_horizontally
						);
				}
				else
				{
					...
					for (int k = half_size; k >= 0; --k)
						set_frame(
							facing, 
							HuneAnimationValues<HUNE_WALKING>::index + k, 
							prefix + "walk" + to_string(k > quarter_size ? half_size - k : k), 
							flip_horizontally
						);
					for (int k = HuneAnimationValues<HUNE_WALKING>::size - 1; k > half_size; --k)
						set_frame(
							facing, 
							HuneAnimationValues<HUNE_WALKING>::index + k, 
							prefix + "walk" + to_string(k > 3 * quarter_size ? HuneAnimationValues<HUNE_WALKING>::size + quarter_size - k : k - quarter_size), 
							flip_horizontally
						);
				}
			}
			...
			void load_asymmetric(HuneDirection facing, std::string prefix, bool flip_horizontally, bool half_displacement, int displacement = 0)
			{
				...
				{
					int mid_index = HuneAnimationValues<HUNE_WALKING>::size / 2;
					for (int k = HuneAnimationValues<HUNE_WALKING>::size - 1; k >= 0; --k)
						set_frame(
							facing, 
							HuneAnimationValues<HUNE_WALKING>::index + ((k + mid_index) % HuneAnimationValues<HUNE_WALKING>::size), 
							prefix + "walk" + to_string(k + displacement), 
							flip_horizontally
						);
				}
				else
			...
			{
				set_frame(facing, frame, HuneSprite::sprite_sheet->get_sprite(sprite_id), flip_horizontally);
			}
			...
			{
				if (frame >= HuneAnimationValues<HUNE_ALL>::size) return;

				HuneSprite* index = m_Sprites + frame + (HuneAnimationValues<HUNE_ALL>::size * (int)facing);
				index->sprite = sprite;
		...
		public:
			...
			{
				if (frame >= HuneAnimationValues<HUNE_ALL>::size) return nullptr;
				return m_Sprites + frame + (HuneAnimationValues<HUNE_ALL>::size * (int)facing);
			}
		};
		...
		int HuneLowerBody::display(HuneDirection facing, int frame) const
		{
			...
			case FACING_FRONT:
				...
				Transform::model.translate(-8.f, 0.f, frame < HuneAnimationValues<HUNE_WALKING>::index + (HuneAnimationValues<HUNE_WALKING>::size / 2) ? -0.01f : 0.01f);
		...
		int HuneUpperBody::display(HuneDirection facing, int frame) const
		{
			...
			case FACING_FRONT:
			{
				float left_dz = frame < HuneAnimationValues<HUNE_WALKING>::index + (HuneAnimationValues<HUNE_WALKING>::size / 2) ? -0.01f : 0.01f;
		...
		{
			return new HuneBasicAnimation(HuneAnimationValues<HUNE_STANDING>::index, HuneAnimationValues<HUNE_STANDING>::size);
		}

		HuneAnimation* generate_hune_walking_animation()
		{
			return new HuneBasicAnimation(HuneAnimationValues<HUNE_WALKING>::index, HuneAnimationValues<HUNE_WALKING>::size);
		}
		...
		*/
		...
		HuneGraphic::HuneGraphic()
		{
			...
			m_Animation = generate_hune_standing_animation();
		}
		...
		{
			if (!head.empty()) m_HeadTexture = HuneSprite::sprite_sheet->get_texture(head);
			if (!arms.empty()) m_BaseUpperBody.arm_texture = HuneSprite::sprite_sheet->get_texture(arms);
			if (!torso.empty()) m_BaseUpperBody.torso_texture = HuneSprite::sprite_sheet->get_texture(torso);
			if (!legs.empty()) m_BaseLowerBody.texture = HuneSprite::sprite_sheet->get_texture(legs);
		}

		void HuneGraphic::set_head_texture(string texture)
		{
			Texture* temp = HuneSprite::sprite_sheet->get_texture("head " + texture);
			m_HeadTexture = temp ? temp : HuneSprite::sprite_sheet->get_texture("monochrome");
		}

		void HuneGraphic::set_body_texture(string texture)
		{
			Texture* temp = HuneSprite::sprite_sheet->get_texture("torso " + texture);
			m_BaseUpperBody.torso_texture = temp ? temp : HuneSprite::sprite_sheet->get_texture("monochrome");

			temp = HuneSprite::sprite_sheet->get_texture("arm " + texture);
			m_BaseUpperBody.arm_texture = temp ? temp : HuneSprite::sprite_sheet->get_texture("monochrome");

			temp = HuneSprite::sprite_sheet->get_texture("leg " + texture);
		...
		void HuneGraphic::set_top_texture(string texture)
		{
			...
			if (regex_match(texture, match, word_separator))
			{
				...
				Texture* temp = HuneSprite::sprite_sheet->get_texture("top torso " + texture);
				m_Top.torso_texture = temp ? temp : HuneSprite::sprite_sheet->get_texture("top torso nude none");

				temp = HuneSprite::sprite_sheet->get_texture("top arm " + texture);
				m_Top.arm_texture = temp ? temp : HuneSprite::sprite_sheet->get_texture("top arm nude none");
			}
		}
		...
		void HuneGraphic::set_jacket_texture(string texture)
		{
			...
			if (regex_match(texture, match, word_separator))
			{
				...
				Texture* temp = HuneSprite::sprite_sheet->get_texture("jacket torso " + texture);
				m_Jacket.torso_texture = temp ? temp : HuneSprite::sprite_sheet->get_texture("jacket torso nude none");

				temp = HuneSprite::sprite_sheet->get_texture("jacket arm " + texture);
				m_Jacket.arm_texture = temp ? temp : HuneSprite::sprite_sheet->get_texture("jacket arm nude none");
			}
		}
		...
		void HuneGraphic::set_bottom_texture(string texture)
		{
			...
			if (regex_match(texture, match, word_separator))
			{
				...
				Texture* temp = HuneSprite::sprite_sheet->get_texture("bottom leg " + texture);
				m_Bottom.texture = temp ? temp : HuneSprite::sprite_sheet->get_texture("bottom leg nude none");
			}
		}
		...
		void HuneGraphic::set_shoe_texture(string texture)
		{
			...
			if (regex_match(texture, match, word_separator))
			{
				...
				Texture* temp = HuneSprite::sprite_sheet->get_texture("shoe leg " + texture);
				m_Shoes.texture = temp ? temp : HuneSprite::sprite_sheet->get_texture("shoe leg nude none");
			}
		}
		...
		void HuneGraphic::display(const vec3i& normal) const
		{
			int frame = m_Animation->get_frame();
			
			// Calculate which way the hune is facing
			// TODO more than 4 directions
			vec2i n = normal;
			Int fc = direction.dot(n); // The dot product
			Int facing_mag = direction.square_sum();
			Int normal_mag = n.square_sum();

			Int angle_mod = 0;
			if (2 * fc * fc > facing_mag * normal_mag)
			{
				angle_mod = fc < 0 ? -1 : 1;
			}

			HuneDirection facing; 
			if (direction.get(0) * normal.get(1) < direction.get(1) * normal.get(0)) // Angle from normal direction to facing direction is in range (180, 360)
			{
				facing = (HuneDirection)(FACING_RIGHT + angle_mod);
			}
			else // Angle from normal direction to facing direction is in range [0, 180]
			{
				facing = (HuneDirection)(FACING_LEFT - angle_mod);
			}
		...
		{
			// TODO more than 4 directions?
			direction = vec2i(direction.get(1), -direction.get(0));
		}
